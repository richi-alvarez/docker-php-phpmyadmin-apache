...
///////////////////////////////////////////////////////////////////////////////
    ...
    if (/order-pay/i.test(page.url())) {
      const finalizeFallback = page.getByRole('button', { name: /Finalizar|Finalize/i }).first();
      if (await finalizeFallback.isVisible({ timeout: 60000 }).catch(() => false)) {
        await expect(finalizeFallback).toBeEnabled({ timeout: 60000 });
        await finalizeFallback.click({ force: true, noWaitAfter: true }).catch(() => {});
      }
    }
    ...
    if (reachedOrderReceived) {
      ...
      const approvedHeading = page.getByRole('heading', { name: /Transacción aprobada|Transaction approved/i }).first();
      const approvedMarker = page.getByText(/Transacción aprobada|Transaction approved|Referencia ePayco|ePayco'?s Reference/i).first();
      const approvedVisible = await Promise.race([
        approvedHeading.waitFor({ state: 'visible', timeout: 30000 }).then(() => true).catch(() => false),
        approvedMarker.waitFor({ state: 'visible', timeout: 30000 }).then(() => true).catch(() => false),
      ]).catch(() => false);

      if (!approvedVisible) {
        testInfo.annotations.push({ type: 'warning', description: 'No se detectó confirmación final explícita en la página principal después del flujo ePayco.' });
      }
