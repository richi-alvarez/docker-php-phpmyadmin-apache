...
namespace EpaycoSubscription\Woocommerce\Funnel;
...
class Funnel
{
    ...
    {
        $this->update(['plugin_version' => EPS_VERSION], $after);
    }
    ...
    private function runWithTreatment(\Closure $callback): void
    {
        try {
            ...
            $GLOBALS['epaycosuscription']->logs->file->error(sprintf("Error on %s\n%s", __METHOD__, $ex), __CLASS__);
        }
    }
}